...
import {
  InboundEmailProviderAdapter,
  CanonicalInboundReply,
  WebhookVerificationContext,
} from '../domain/inbound-email-provider.adapter';
...
export class ResendInboundEmailAdapter extends InboundEmailProviderAdapter {
  ...
  parsePayload(
    ...
    if (payload.type !== 'email.received') {
      throw new AppValidationException(
        `Unsupported webhook event type: ${payload.type}`,
      );
    }
    ...
    if (!data.email_id)
      throw new AppValidationException('Missing email_id in payload');
    if (!data.message_id)
      throw new AppValidationException('Missing message_id in payload');
    if (!data.from)
      throw new AppValidationException('Missing from address in payload');
    ...
    if (!toField)
      throw new AppValidationException('Missing recipient in payload');
    ...
    return {
      ...
      receivedAt: (() => {
        ...
        if (isNaN(d.getTime()))
          throw new AppValidationException('Invalid created_at timestamp');
        return d;
      })(),
    };
  }
