...
import { IInboundEmailContentAdapterRegistry, InboundEmailContentAdapter } from '../domain/inbound-email-content.adapter';
...
export class InboundEmailContentAdapterRegistry implements IInboundEmailContentAdapterRegistry {
  constructor(
    private readonly resendAdapter: ResendInboundContentAdapter,
  ) {}

  getAdapter(provider: string): InboundEmailContentAdapter<ProviderCredentials> {
    if (provider === 'RESEND') {
      return this.resendAdapter as any;
    }
    
    throw new AppValidationException(`Provider ${provider} does not support inbound content retrieval`);
  }
}
