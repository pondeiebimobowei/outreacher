...
export class InboundRetrievalException extends AppException {
  constructor(
    message: string,
    public readonly retrievalCode: InboundRetrievalErrorCode,
    public readonly isRetryable: boolean,
    statusCode: number = HttpStatus.BAD_GATEWAY,
  ) {
    super(message, statusCode, ErrorCode.PROVIDER_FAILURE, {
      code: retrievalCode,
      isRetryable,
    });
  }
...
export class ResendInboundContentAdapter implements InboundEmailContentAdapter<ResendCredentials> {
  ...
  async getEmailDetails(
    ...
    if (!credentials || !credentials.apiKey) {
      throw new InboundRetrievalException(
        'Missing Resend credentials',
        InboundRetrievalErrorCode.CREDENTIAL_ERROR,
        false,
    ...
    try {
      ...
      const { data, error } = await resend.get(path, {
      ...
      if (error) {
        ...
        if (error.statusCode === 404) {
          throw new InboundRetrievalException(
            ...
            InboundRetrievalErrorCode.NOT_FOUND,
            false,
            error.statusCode,
          );
        }
        if (error.statusCode && error.statusCode >= 500) {
          throw new InboundRetrievalException(
            `Provider temporary error: ${error.message}`,
        ...
        if (error.statusCode === null || !error.statusCode) {
          throw new InboundRetrievalException(
            ...
            InboundRetrievalErrorCode.CONNECTIVITY_ERROR,
            true,
          );
        }
      ...
      const headers = Object.keys(rawHeaders).reduce(
        (acc, key) => {
      ...
      if (
        ...
        throw new InboundRetrievalException(
          ...
          InboundRetrievalErrorCode.TIMEOUT,
          true,
        );
      }
      throw new InboundRetrievalException(
        `Unknown error retrieving email: ${err.message}`,
        InboundRetrievalErrorCode.PROVIDER_ERROR,
        true,
      );
    } finally {
