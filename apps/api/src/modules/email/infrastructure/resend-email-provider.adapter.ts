...
export class EmailProviderException extends AppException {
  constructor(
    message: string,
    public readonly dispatchErrorCode: EmailDispatchErrorCode,
    details?: Record<string, unknown>,
  ) {
    super(message, HttpStatus.BAD_GATEWAY, ErrorCode.PROVIDER_FAILURE, {
      ...details,
      dispatchErrorCode,
    });
  }
...
export class ResendEmailProviderAdapter implements EmailProviderAdapter<ResendCredentials> {
  ...
  async sendEmail(
    input: SendEmailInput<ResendCredentials>,
    ...
    const finalReplyTo = `reply+${input.replyToToken}@${replyDomain}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 409 && parsedName === 'conflict') {
          ...
          if (!messageIdFromConflict) {
            throw new EmailProviderException(
              'Idempotency conflict but no message ID returned',
              EmailDispatchErrorCode.PROVIDER_IDEMPOTENCY_CONFLICT,
              { statusCode: 409 },
            );
          }
          return {
            providerMessageId: messageIdFromConflict,
      ...
      if (error.name === 'AbortError') {
        throw new EmailProviderException(
          'Provider request timed out post-submit',
      ...
      if (error.cause) {
        ...
        if (
          ...
          throw new EmailProviderException(
            `Pre-submit connection failure: ${sysCode}`,
        ...
        if (['ECONNRESET', 'EPIPE', 'ETIMEDOUT'].includes(sysCode)) {
          ...
          throw new EmailProviderException(
            `Post-submit socket failure: ${sysCode}`,
            EmailDispatchErrorCode.PROVIDER_TIMEOUT_UNCERTAIN,
          );
        }
      }
      ...
      throw new EmailProviderException(
        `Email provider dispatch failed: ${error?.message || 'Unknown'}`,
        EmailDispatchErrorCode.PROVIDER_TIMEOUT_UNCERTAIN,
      );
    } finally {
