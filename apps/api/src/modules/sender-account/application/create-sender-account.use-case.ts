...
import { AppValidationException, AppNotFoundException, AppConflictException } from '../../../common/errors/application.exception';
...
export class CreateSenderAccountUseCase {
  ...
  public async execute(workspaceId: string, dto: CreateSenderAccountDto) {
    ...
    if (!integration) {
      throw new AppValidationException('Integration does not exist or does not belong to this workspace');
    }

    const canonicalFromEmail = dto.fromEmail.trim().toLowerCase();
    const canonicalReplyTo = dto.replyTo ? dto.replyTo.trim().toLowerCase() : null;
