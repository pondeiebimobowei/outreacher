...
describe('SelectContactUseCase', () => {
  ...
  it('rejects if target company does not exist in workspace', async () => {
    ...
    await expect(useCase.execute('ws-1', 'comp-1', 'cont-1')).rejects.toThrow(AppNotFoundException);
  });

  it('rejects if contact does not exist in workspace', async () => {
    prisma.company.findFirst.mockResolvedValue({ id: 'comp-1', workspaceId: 'ws-1' });
    prisma.contact.findFirst.mockResolvedValue(null);

    await expect(useCase.execute('ws-1', 'comp-1', 'cont-99')).rejects.toThrow(AppNotFoundException);
  });
  ...
  it('rejects cross-entity selection when contact belongs to a different company', async () => {
    ...
    await expect(useCase.execute('ws-1', 'comp-1', 'cont-1')).rejects.toThrow(AppForbiddenException);
  });
  ...
  it('successfully upserts selection when contact belongs to target company and workspace', async () => {
    ...
    expect(repository.setCompanyContactSelection).toHaveBeenCalledWith('ws-1', 'comp-1', 'cont-1');
  });
});
