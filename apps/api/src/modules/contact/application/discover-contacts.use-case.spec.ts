...
describe('DiscoverContactsUseCase', () => {
  ...
  it('throws AppNotFoundException if company does not exist in workspace', async () => {
    ...
    await expect(useCase.execute('ws-1', 'comp-99')).rejects.toThrow(AppNotFoundException);
  });

  it('reuses 24-hour fresh completed discovery job when forceRefresh is false', async () => {
    prisma.company.findFirst.mockResolvedValue({ id: 'comp-1', workspaceId: 'ws-1', name: 'Acme' });
    ...
    const result = await useCase.execute('ws-1', 'comp-1', { forceRefresh: false });
  ...
  it('deduplicates to active PENDING or RUNNING job if present', async () => {
    ...
    const result = await useCase.execute('ws-1', 'comp-1', { forceRefresh: true });
  ...
  it('enforces 3 forced refresh / 24h / workspace limit', async () => {
    ...
    await expect(useCase.execute('ws-1', 'comp-1', { forceRefresh: true })).rejects.toThrow(AppRateLimitException);
  });
  ...
  it('enqueues new job when under forced refresh rate limit', async () => {
    ...
    prisma.job.create.mockResolvedValue({ id: 'job-300', status: JobStatus.PENDING });
