...
import { CampaignContact } from '@repo/db';
...
export class AddCampaignContactsUseCase {
  ...
  async execute(
    ...
    const resolvedContacts = await Promise.all(
      uniqueRequestedIds.map((contactId) =>
        this.contactRepository.findContactById(workspaceId, contactId),
      ),
    );
    ...
    for (let i = 0; i < uniqueRequestedIds.length; i++) {
      if (!resolvedContacts[i]) {
        throw new AppNotFoundException('Contact');
      }
    }
