...
import {
  SendEmailUseCase,
  SendEmailCommand,
} from '../../src/modules/email/application/send-email.use-case';
...
} from '../../src/modules/email/infrastructure/resend-email-provider.adapter';
...
import {
  ...
  SendEmailInput,
  SendEmailResult,
} from '../../src/modules/email/domain/email-provider.adapter';
...
describe('Packet 9C: Outbound Dispatch & Provider Adapter', () => {
  ...
  it('2. Capacity Race & Serialization Test', async () => {
    ...
    expect((failures[0] as any).reason.message).toContain(
  ...
  it('4. PROVIDER_TIMEOUT_UNCERTAIN Dead-Letter Test', async () => {
    ...
    expect(emailSend?.errorCode).toBe(
      EmailDispatchErrorCode.PROVIDER_TIMEOUT_UNCERTAIN,
    );
  ...
  it('5. Resend Idempotency 409 Replay Test', async () => {
    ...
    global.fetch = jest.fn().mockResolvedValue({
      ...
      status: 409,
      text: async () =>
        JSON.stringify({ name: 'conflict', id: 'resend-recovered-id' }),
    }) as any;
  ...
  it('9. Stale job recovery lease-safety race condition', async () => {
    ...
    const finalEmailSend = await prisma.emailSend.findUnique({
      where: { id: payload.emailSendId },
    });
    ...
    global.fetch = jest.fn().mockImplementation(async (url, init) => {
      ...
      if (acceptedKeys.has(idempotencyKey)) {
        return {
          ...
          status: 409,
          text: async () =>
            JSON.stringify({ name: 'conflict', id: 'resend-recovered-id' }), // Resend returns 409
        };
      }
      ...
      if (!sweeperRan) {
        ...
        expect(claimedB!.job.leaseVersion).toBeGreaterThan(
          claimedA!.job.leaseVersion,
        ); // Higher lease
