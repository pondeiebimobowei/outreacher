...
import { Building2, Users, Search, ArrowRight, MapPin, Globe } from 'lucide-react';
...
function CompanyContactPortalCard({ company }: { company: CompanyDto }) {
  ...
  return (
    <div className="p-4 sm:p-5 bg-white rounded-xl border border-slate-200 hover:border-slate-300 hover:shadow-sm transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div className="flex items-start sm:items-center gap-4 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1.5 flex-wrap text-xs text-slate-500">
            {company.industry && (
              <span className="inline-flex items-center gap-1 font-medium bg-slate-50 text-slate-600 border border-slate-200 px-2 py-0.5 rounded">
      ...
      <div className="shrink-0 flex items-center sm:self-center">
        <Link
          ...
          className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 hover:text-slate-900 border border-slate-200/80 transition-all focus:outline-none focus:ring-2 focus:ring-slate-900"
          style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}
        >
...
export function ContactsIndexComponent() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
      ...
      {/* Info note */}
      <div className="mb-6 p-4 rounded-xl border border-indigo-100 bg-indigo-50/70 text-indigo-950 flex items-start gap-3">
      ...
      {companies && companies.length > 0 && (
        <div className="mb-5 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-slate-200 bg-white text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-slate-900 transition-all"
          />
        </div>
      )}
