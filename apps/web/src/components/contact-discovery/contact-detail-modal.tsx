  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        tabIndex={-1}
        className="bg-white rounded-xl border border-slate-200 shadow-xl max-w-lg w-full p-6 space-y-5 focus:outline-none animate-in fade-in zoom-in-95 duration-150"
      >
        {/* Header: Identity, Title, Badges, Close */}
        <div className="flex items-start justify-between border-b border-slate-100 pb-4">
          <div className="space-y-1">
            <div className="flex items-center space-x-2">
              <h2 id="contact-detail-modal-title" className="text-lg font-bold text-slate-900">
                {contact.name}
              </h2>
              <span
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            className="text-slate-400 hover:text-slate-600 p-1 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-slate-900"
          >
            ✕
          </button>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-700 leading-relaxed">
            <span className="font-bold text-slate-900 block mb-1">Why This Contact:</span>
        ...
        <div className="space-y-2 border-t border-slate-100 pt-3">
          ...
          {isEmailAvailable ? (
            <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-xs flex items-center justify-between text-emerald-900 font-mono">
              <span className="font-semibold">Email Channel:</span>
              <span className="font-bold">{contact.email}</span>
            </div>
          ) : (
            <div className="p-3 bg-slate-100 border border-slate-200 rounded-lg text-xs space-y-1 text-slate-700">
              <div className="flex items-center space-x-2">
                <span className="font-semibold">Email Channel:</span>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 pt-4">
          ...
          {isSelected ? (
            <span className="px-4 py-2 text-xs font-bold text-emerald-700 bg-emerald-100 border border-emerald-300 rounded-md inline-flex items-center space-x-1">
